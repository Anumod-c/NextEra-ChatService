...
export default class Consumer{
    ...
    async consumeMessage(){
        try{
            ...
            await this.channel.assertQueue(RabbitMQconfig.rabbitMQ.queues.ChatQueue,{durable:true});
            this.channel.consume(RabbitMQconfig.rabbitMQ.queues.ChatQueue,async(message:ConsumeMessage|null)=>{
                if(message){
                    const {correlationId,replyTo} = message.properties;
                    const operation =  message.properties.headers?.function;
                    rabbitMQLogger.emit('messageReceived', {
                        queue: RabbitMQconfig.rabbitMQ.queues.ChatQueue,
                        correlationId,
                        operation,
                    });
